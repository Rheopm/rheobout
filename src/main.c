...
int main(u16 hard) /************** MAIN **************/
{
    ...
    while(TRUE) /// LOOP PRINCIPAL ///
    {
		...
		if(gRoom==10) //IN GAME -------------------------------------------------------------------
		{
			...
			// 	if(gPing10 == 9)
			// 	{
			// 		VDP_setTileMapEx(BG_B,gfx_bgb4.tilemap,TILE_ATTR_FULL(PAL0,0,FALSE,FALSE,0),18,8,18,(28*img_num+8),48,5,DMA_QUEUE);
			// 		img_num++;
			// 		if(img_num == 4)
			// 		{
			// 			img_num = 0;
			...
			if(gBG_Choice==1)
			{
				if(gPing10 == 9)
				{
					VDP_setTileMapEx(BG_B,gfx_bgb1.tilemap,TILE_ATTR_FULL(PAL0,0,FALSE,FALSE,0),0,12,0,(28*img_num+12),64,6,DMA_QUEUE);
					img_num++;
					if(img_num == 4)
					{
						img_num = 0;
...
void FUNCAO_PLAY_SND(u8 Player, u16 State)
{
	...
	if(Player==1)
	{
		...
		if(P[1].id==3) //bluemary
		{
			...
			if(State==611){ XGM_setPCM(P1_SFX, snd_bluemary_611, sizeof(snd_bluemary_611)); XGM_startPlayPCM(P1_SFX, 1, SOUND_PCM_CH3); }
		}
	}
	...
	if(Player==2)
	{
		...
		{		
			...
			if(State==610) { XGM_setPCM(P2_SFX, snd_terry_610, sizeof(snd_terry_610)); XGM_startPlayPCM(P2_SFX, 1, SOUND_PCM_CH4); }
		}
		if(P[2].id==2) //Mai
		{		
			if(
			...
			if(State==610){ XGM_setPCM(P2_SFX, snd_mai_610, sizeof(snd_mai_610)); XGM_startPlayPCM(P2_SFX, 1, SOUND_PCM_CH4); }
			if(State==612){ XGM_setPCM(P2_SFX, snd_mai_612, sizeof(snd_mai_612)); XGM_startPlayPCM(P2_SFX, 1, SOUND_PCM_CH4); }
			if(State==613){ XGM_setPCM(P2_SFX, snd_mai_612, sizeof(snd_mai_612)); XGM_startPlayPCM(P2_SFX, 1, SOUND_PCM_CH4); }
		}
